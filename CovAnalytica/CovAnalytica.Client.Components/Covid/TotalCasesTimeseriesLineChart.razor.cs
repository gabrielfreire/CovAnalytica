...
{
	public partial class TotalCasesTimeseriesLineChart : ComponentBase
	{
        [Inject]
        ...
        public Dataset BuildDataset(List<CompleteCovidData> covidDataList, string color = null)
        {
            ...
            _dataset.Items = covidDataList.Where(it => it.NewCasesPerMillion != null && it.NewCasesPerMillion > 0)
                .Select(it => new DataItem() { Date = it.Date, Value = decimal.Round(it.NewCasesPerMillion ?? 0) }).ToArray();
